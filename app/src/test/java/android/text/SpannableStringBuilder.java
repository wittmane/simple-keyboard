...
public class SpannableStringBuilder implements CharSequence, GetChars, Spannable, Editable,
     ...
     */
    private <T> T[] getSpans(int queryStart, int queryEnd, Class<T> kind,
    ...
    private <T> int getSpansRec(int queryStart, int queryEnd, Class<T> kind,
                                int i, T[] ret, int[] priority, int[] insertionOrder, int count,
                                boolean sort) {
        if ((i & 1) != 0) {
            // internal tree node
            int left = leftChild(i);
    ...
    //TODO: (EW) consider removing these and just use ArrayLists
